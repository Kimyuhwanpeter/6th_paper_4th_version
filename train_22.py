...
FLAGS = easydict.EasyDict({"img_size": 384,
                           ...
                           "pre_checkpoint_path": "/yuhwan/yuhwan/checkpoint/Segmenation/6th_paper/Apple_A/checkpoint/399",
...
def tversky_coef(y_true, y_pred, alpha=0.5):
...
def focal_tversky(y_true, y_pred, alpha=0.5, gamma=0.75):
    ...
    loss_val = tf.keras.backend.pow((1-tversky_coef(y_true, y_pred, alpha=alpha)), gamma)
...
def cal_loss(model, images, labels, class_imbal_labels_buf, objectiness, object_buf, crop_buf, weed_buf):
...
def main():
    ...
    model = modified_Unet_PP(input_shape=(FLAGS.img_size, FLAGS.img_size, 3), nclasses=1)
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                loss = cal_loss(model, batch_images, batch_labels, class_imbal_labels_buf, objectiness, object_buf, crop_buf, weed_buf)
                if count % 10 == 0:
                ...
                if count % 100 == 0:
                    ...
                    for i in range(FLAGS.batch_size):
                        ...
                        crop_weed_image[background_indices] = 2
